...
As a direct result, the integrity-checking core is **well-isolated and resistant to most hook-based attacks**: replacing `window.fetch`, `Array.prototype.push`, `String.prototype.toLowerCase`, or any other live global after page load cannot change the guard's behaviour — it uses pure operators for all string processing, validates captured references on every watchdog tick, and would detect the replacement before an attacker could leverage it.
